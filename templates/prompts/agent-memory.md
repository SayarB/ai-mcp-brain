...
| `jira_assigned` | Optional Jira fetch/cache. Default open sprints; `scope=all` only after asking. `refresh` busts 1h cache. Does not edit today. |
| `work_today` | Vault today list: `list` / `add` / `complete` (no Jira). Prefer over raw file edits. |
| `work_plate` | Compare Jira cache vs today. Default sprint scope. Never auto-add. |
...
Default Jira scope is **open sprints only**. Ask before using `scope=all` (every assigned not-Done issue). Ask yes/no before ticking a matched today item. Never silent-tick. Never ambient Jira.
---
...
Orchesto personas (`persona-cpo` optional / `persona-architect` / `persona-implementor` / `persona-reviewer`) live under `workflows/global/` (optional project overlays). They are **not** actions. The pipeline skill is installed **with Hem Vault** (`setup` / `inject`) into global harness dirs: `~/.cursor/skills/orchesto/`, `~/.agents/skills/orchesto/`, `~/.claude/skills/orchesto/`. Repair / extras playbook: `workflows/global/setup-orchesto.md`. Orchesto **always asks** whether a PRD / CPO pass is needed before architect; seat CPO only on user yes. Do not add an `orchesto` action. Optional pre-step: user may seat **brainstormer** before CPO/architect (not auto-run). Architect may **suggest capability phases** on a large ask; if the user agrees, all phase `plan.md` / `validations.md` files are written in one sitting under `.plans/<slug>/<NN>-<phase>/`. Approving **that** phase’s plan is the go to build it. Implementor/reviewer use the **current phase folder** only. **Do not auto-start** the next phase after review `pass`.
